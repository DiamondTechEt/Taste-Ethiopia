...
import { Plus, X, ArrowLeft, Camera, Image as ImageIcon, Upload } from 'lucide-react-native';
...
export default function EditRecipeScreen() {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (!recipe) {
        Alert.alert(t('Error', 'ስህተት'), t('Recipe not found', 'ምግቡ አልተገኘም'));
        router.back();
        return;
      }

      if (!isCustomRecipe(id)) {
        Alert.alert(t('Error', 'ስህተት'), t('Cannot edit built-in recipes', 'የተሰሩ ምግቦችን ማስተካከል አይቻልም'));
        router.back();
        return;
      }
      ...
      setTitleAmharic(recipe.titleAmharic);
      setDescription(recipe.description);
      setDescriptionAmharic(recipe.descriptionAmharic);
      ...
      setTips(recipe.tips || '');
      setTipsAmharic(recipe.tipsAmharic || '');
      setRecipeImage(recipe.image || null);
    }
  }, [id, getRecipeById, isCustomRecipe, router, t]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert(
        t('Success', 'ተሳክቷል'),
        t('Recipe updated successfully!', 'ምግቡ በተሳካ ሁኔታ ተሻሽሏል!'),
        [{ text: t('OK', 'እሺ'), onPress: () => router.back() }]
      );
    } catch (error) {
      Alert.alert(t('Error', 'ስህተት'), t('Failed to update recipe', 'ምግቡን ማሻሻል አልተቻለም'));
    }
  };
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              {t('Update Recipe Photo', 'የምግቡ ፎቶ አዘምን')}
            </Text>
